...
(() => {
  ...
  function getImageUrlFromElement(target) {
    ...
    if (element.dataset.src || element.dataset.original) {
      return normalizeImageUrl(element.dataset.src || element.dataset.original);
    }
    ...
    if (bgImage && bgImage !== 'none' && bgImage.startsWith('url(')) {
      ...
      if (urlMatch) {
        return normalizeImageUrl(urlMatch[1]);
      }
    }
    ...
    while (parent && parent !== document.body) {
      ...
      if (parentImg) {
        return normalizeImageUrl(
          parentImg.dataset.src ||
          parentImg.dataset.original ||
          parentImg.dataset.lazy ||
          parentImg.currentSrc ||
          parentImg.src
        );
      }
