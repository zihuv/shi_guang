...
export const VIEWPORT_OVERSCAN_PX = 600
      ...
      case "ArrowUp": {
        ...
        const overlap = Math.max(
          ...
          Math.min(currentRight, candidateRight) -
            Math.max(currentLeft, candidateLeft),
        )
      ...
      case "ArrowLeft": {
        ...
        const overlap = Math.max(
          ...
          Math.min(currentBottom, candidateBottom) -
            Math.max(currentTop, candidateTop),
        )
        const rank: [number, number, number, number] = [
          overlap > 0 ? 0 : 1,
...
export function buildAdaptiveLayout(
  ...
  const items: AdaptiveLayoutItem[] = files.map((file, index) => {
    ...
    const totalHeight =
      imageHeight + getAdaptiveFooterHeight(file, visibleFields)
    let columnIndex = 0
...
export function buildAdaptiveColumns(
  items: AdaptiveLayoutItem[],
  columns: number,
) {
